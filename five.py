...
def gamemain():
    while True:
        ...
        print("9.人机对弈(强化学习算法).")
        print("10.比较博弈树算法和强化学习算法.")
        print("11.退出.")
        choice = input("请输入您的选择:")
        # print(choice, type(choice), choice.isdigit(), int(choice))
        if choice.isdigit() == False or int(choice) > 11:
            print("输入错误请重新输入")
            input("按任意键继续")
            continue
        choice = int(choice)
...
def P2RL():
    b = chessboard()
    b.reset()
    result = contest(putchess, RLput, show = True)
    if result == 1:
        print("您赢了！")
    elif result == 2:
        print("计算机赢了!")
...
def compareTreeRL():
    ...
    for i in tqdm.tqdm(range(epochs)):
        b.reset()
        result = contest(treePut, RLput, show = bShow, bTree = True, depth1 = depth1, depth2 = n_playout)
        if result != -1:
            win[result-1] += 1
            N += 1
        # print("第%d次对弈，%d取胜" % (i+1, result))
